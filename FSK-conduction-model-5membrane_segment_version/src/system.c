...
inline void comp_ina(int site, int patch)
{
	...
	if(var.u[0][site][patch]<-95.0){ var.u[1][site][patch]=0.0;}
	if(var.u[0][site][patch]<-120.0){ var.u[2][site][patch]=1.0;}
	if(var.u[0][site][patch]<-220.0){ var.u[3][site][patch]=1.0;}
...
inline void conc_jsr (int site, int patch)
{
	...
	fexp=exp((-var.t_cicr[site][patch]+4.0)/0.5);
	on = 1.0/(1.0+fexp);
