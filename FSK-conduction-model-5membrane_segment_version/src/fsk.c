...
/* Action Potential Duration and Max. Info */
	...
	if ((fp2 = fopen("vm_data.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp3 = fopen("Ina_data.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	} 
	if ((fp4 = fopen("ndata.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp5 = fopen("ndata_final.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
/*	if ((fp6 = fopen("inner_current.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp7 = fopen("cleft_radial_potential.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp8 = fopen("cleft_axial_potential.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}*/
	if ((fp9 = fopen("check_ndata_final.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
/*	if ((fp10 = fopen("internal_potential.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	} 
	...
	if ((fp12 = fopen("total_current.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp13 = fopen("ito.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp14 = fopen("iki.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp15 = fopen("iks.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp16 = fopen("ikr.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	} 
	if ((fp17 = fopen("inak.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp18 = fopen("inaca.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	} 
	if ((fp19 = fopen("dvdt.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp20 = fopen("cai.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	if ((fp21 = fopen("ina_h.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	} */
	...
	if ((fp25 = fopen("act_time.out","w")) == NULL){
		printf("Can't open File\n");
		exit(1);
	}
	...
	// surface area of junctional and non-Junctional membran unit	
		...
		switch(var.couple_type){
			case 0: // Gap junction alone
				...
				var.p1 = 0.0;
				break;
			case 1:	// Gap+EF junction
				var.q1 = var.gx1*(0.5*var.Rd+var.Rj);
				var.q2 = var.gx1*var.Rj;
				var.p1 = var.gg*0.5*var.Rd;
				break;
		}

		printf("gx1=%e, gx2=%e, ggx=%e\n",var.gx1,var.gx2,var.gg);
		printf("gx1*(Rj+0.5Rd)=%e, gx1*Rj=%e, ggx*Rd/2=%e\n",var.q1,var.q2,var.p1);
		printf("radius = %e\n",var.a);
		...
		fprintf(fp1,"Ri = %e, (Gi = %e)\n",var.Ri,M_PI*var.a*var.a/(var.Rmyo*var.length));
		fprintf(fp1,"Rg = %e, (Gg = %e)\n",var.Rg,var.gg);
		fprintf(fp1,"gx1*(Rj+0.5Rd)=%e, gx1*Rj=%e, ggx*Rd/2=%e\n",var.q1,var.q2,var.p1);
		fprintf(fp1,"Ena=%f, Ek=%f\n",log(var.u[21][0][1]/var.u[16][0][1])*var.RTonF,log(var.u[22][0][1]/var.u[17][0][1])*var.RTonF);

	// for solving algebra equation.
		var.par_k = 11*(MEDIA_SITE-2)+8*2;
		printf("var.par_k = %d\n",var.par_k);
	...
	// Initialization time
			...
			/*	for(k=0;k<MEDIA_SITE;k++){
				...
				if (time>= (beats-2)*var.BCL && time <= beats*var.BCL){
					...
					if(j%(int)cut == 0){ // To reduce the recording data.
						ina_data(fp3,fp24,time);
						//ina_data(fp3,time);
						//ina_data(fp3,fp17,fp18,time);
						//ik_data(fp13,time);
						//ilca_data(fp11,time);
						//cleft_v_data(fp7,fp8,time);
						//dvdt_data(fp19,time);
						//cai_data(fp20,time);
					}
				}
		...
		for(z=0;z<beats;z++){
			...
			for(m=0;m<MEDIA_SITE;m++){
				for(k=0;k<MEDIA_PATCH;k++){
					fprintf(fp4,"%i\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%e\n",
		...
		for(m=0;m<MEDIA_SITE;m++){
			for(k=0;k<MEDIA_PATCH;k++){
				fprintf(fp9,"%d-%d-%d\n",beats-1,m,k);
				fprintf(fp5,"%i\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%e\n",
...
/* -------------------------------------------------------------------- */
		...
		fclose(fp9);
		//fclose(fp10);
		//fclose(fp11); fclose(fp13);
		//fclose(fp17); fclose(fp18);
		//fclose(fp19); fclose(fp20);
		fclose(fp22); 
		fclose(fp23); fclose(fp24); 
		fclose(fp25);
